...
//-----------------objects------------------
// balancer object BMS = battery management system
bq769x0 BMS(bq76920, BMS_I2C_ADDRESS);    			
//PI - controller				
PID myPID(&Input, &duty_cycle, &Setpoint, V_KP,V_KI, KD, DIRECT);		
...
void setup() {
	...
	int err = BMS.begin(BMS_ALERT_PIN, BMS_BOOT_PIN);
  	...
  	// minIdleTime_min, minCellV_mV, maxVoltageDiff_mV
  	BMS.setBalancingThresholds(0, 3300, 20);  			
  	BMS.setIdleCurrentThreshold(100);
  	//let balancer handle balancing
...
void guess_soc(){
	// taken from "Battery management for an off-grid PV system using flooded lead-acid batteries"
	la_soc = 0.087 * la_v - 1000; 
	last_soc = millis();
}
	...
	if(la_state == LA_FLOAT){
		...
		if(la_v <= (V_ABS-100) & la_state != LA_ABSORP){
			...
			if(la_state == LA_EQUALIZE){
				//added duty cycle otherwise unwanted execution
				if(la_i_average<=LA_I_MIN_ABSORP & duty_cycle < 200){ 
					la_soc = 100;
					la_state = LA_FLOAT;
					floatmode();
				}else{
...
void pv_control(){
	//execute PI calculation, will alter duty_cycle if necessary
	myPID.Compute();			
...
void print_data(){
	...
	//only print if last print was PRINT_INTERVAL millisec ago
